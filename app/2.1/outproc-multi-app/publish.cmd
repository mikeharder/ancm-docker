...
for /l %%i in (1,1,%num_apps%) do (
    ...
    rmdir /s /q %ancm%\apps\%publish_path%\%%i
    dotnet publish -c Release -o %ancm%\apps\%publish_path%\%%i %~dp0..\app.csproj

    : Create folder for stdout logs and give IIS full permissions
    mkdir %ancm%\apps\%publish_path%\%%i\logs
    docker exec -it %container% cmd /c icacls C:\apps\%publish_path%\%%i\logs /grant "IIS APPPOOL\DefaultAppPool":F

    : Register app with IIS
    docker exec -it %container% cmd /c %appcmd% add app /site.name:"Default Web Site" /path:/%http_path%/%%i /physicalPath:C:\apps\%publish_path%\%%i
)
